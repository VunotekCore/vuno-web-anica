...
function disable_wp_frontend(){
	// si es una peticion de API o de administracion, permitir
	if ( is_admin() || strpos( $_SERVER['REQUEST_URI'], '/wp-json/' ) === 0 ) {
		return;
	}
	// redirect to another page
	wp_redirect('https://anicasolucionesintegrales.com',301);
	exit;
}
add_action( 'template_redirect', 'disable_wp_frontend' );

function expose_custom_logo_in_rest() {
    // Buscar un logo por nombre o metadato personalizado
    $args = [
    ...
    if ($query->have_posts()) {
        $query->the_post();
        $logo_url = wp_get_attachment_url(get_the_ID());
        wp_reset_postdata();
    } else {
        // fallback: buscar por nombre de archivo
        $maybe_logo = get_page_by_title('logo', OBJECT, 'attachment');
        if ($maybe_logo) {
            $logo_url = wp_get_attachment_url($maybe_logo->ID);
        }
    }

    // Obtener la informacion basica del sitio
    $site_info = [
...
// Agregar el campo content_plain
function get_posts_by_category_slug($request) {
    $slug = sanitize_text_field($request['slug']);

    // 1. Obtener la categoría por slug (interno, no REST)
    ...
    foreach ($query->posts as $post) {
        $posts[] = [
            ...
            'excerpt_plain' => wp_strip_all_tags(get_the_excerpt($post)),
            'content_plain' => apply_filters('the_content', $post->post_content),
...
add_action('rest_api_init', function () {
    ...
    foreach ($post_types as $type) {
        register_rest_field($type, 'content_plain', [
            'get_callback' => function ($post_arr) {
                return wp_strip_all_tags(
                    apply_filters('the_content', $post_arr['content']['raw'])
                );
            },
        ]);
    }
...
// consumir posts por categoria usando los slugs
add_action('rest_api_init', function () {
    register_rest_route('categories/v1', '/posts-by-category/(?P<slug>[a-zA-Z0-9-_]+)', [
        'methods'  => 'GET',
...
// Agregar headers CORS antes de registrar la ruta
add_action('rest_api_init', function() {
    ...
    register_rest_route('custom/v1', '/site-info', [
        'methods' => 'GET',
        'callback' => 'expose_custom_logo_in_rest',
